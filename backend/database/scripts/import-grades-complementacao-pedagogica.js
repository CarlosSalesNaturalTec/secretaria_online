/**
 * Script: import-grades-complementacao-pedagogica.js
 * Descrição: Importa notas de avaliações dos cursos de Complementação Pedagógica
 *            da tabela boletim_novo do banco legado
 *
 * Origem: database/academico_bo.sql
 * Tabela: boletim_novo
 * Filtro: semestre LIKE 'Complementação Pedagógica%'
 *
 * Cursos suportados:
 * - Complementação Pedagógica em Geografia (ID: 9)
 * - Complementação Pedagógica em Letras com Inglês (ID: 10)
 * - Complementação Pedagógica em Matermática (ID: 11)
 * - Complementação Pedagógica em Pedagogia (ID: 12)
 *
 * Semestres no legado: I° e II° (convertidos para 1 e 2)
 *
 * Mapeamento:
 ...
 * - boletim_novo.semestre -> extraction course + semester
 *
 * Como executar:
 * cd backend
 * node database/scripts/import-grades-complementacao-pedagogica.js
 */
...
// Mapeamento de cursos de Complementação Pedagógica
const COURSES = {
  'Geografia': { id: 9, name: 'Complementação Pedagógica em Geografia' },
  'Letras com Inglês': { id: 10, name: 'Complementação Pedagógica em Letras com Inglês' },
  'Matermática': { id: 11, name: 'Complementação Pedagógica em Matermática' },
  'Pedagogia': { id: 12, name: 'Complementação Pedagógica em Pedagogia' }
};

/**
 * Extrai informações do campo "semestre" do banco legado
 * Exemplo: "Complementação Pedagógica em Pedagogia I°" -> { course: 'Pedagogia', semester: 1 }
 *          "Complementação Pedagógica em Letras com Inglês II°" -> { course: 'Letras com Inglês', semester: 2 }
 *
 * @param {string} semestre - Campo semestre do banco legado
 * @returns {Object|null} { course: string, semester: number } ou null
 */
function extractCourseAndSemester(semestre) {
  if (!semestre) return null;

  // Padrão: Complementação Pedagógica em <CURSO> <SEMESTRE>°
  const match = semestre.match(/Complementação Pedagógica em (.+?)\s+(I{1,2})°/i);
  if (match) {
    const courseName = match[1].trim();
    const roman = match[2].toUpperCase();

    // Converte romano para número: I -> 1, II -> 2
    const semester = roman === 'I' ? 1 : roman === 'II' ? 2 : null;

    if (semester && COURSES[courseName]) {
      return {
        course: courseName,
        semester: semester
      };
...
function parseBoletimNovoInserts(sqlContent) {
  ...
  while ((match = insertPattern.exec(sqlContent)) !== null) {
    ...
    while ((valueMatch = valuePattern.exec(valuesBlock)) !== null) {
      ...
      // Filtra apenas registros de Complementação Pedagógica
      if (record.semestre && /Complementação Pedagógica em .+?\s+I{1,2}°/i.test(record.semestre)) {
        const courseInfo = extractCourseAndSemester(record.semestre);
        if (courseInfo) {
          records.push(record);
        }
      }
 ...
 *
 * @param {number} courseId - ID do curso
 * @param {number} semester - Semestre (1-2)
 * @returns {Promise<Object>} Turma criada ou encontrada
 */
async function getOrCreateDefaultClass(courseId, semester) {
  ...
  const courseName = Object.values(COURSES).find(c => c.id === courseId)?.name || 'Complementação Pedagógica';
  console.log(`  ✓ Turma criada: ${courseName} - ${semester}° semestre - ${currentYear}`);
...
async function importGrades() {
  console.log('\n========================================');
  console.log('IMPORTAÇÃO DE NOTAS - COMPLEMENTAÇÃO PEDAGÓGICA');
  console.log('========================================\n');
  ...
  try {
    ...
    // 3. Busca professor default
    console.log('3. Buscando professor para importação...');
    const defaultTeacher = await getOrCreateDefaultTeacher();
    console.log(`   ✓ Professor: ${defaultTeacher.nome} (ID: ${defaultTeacher.id})\n`);

    // 4. Agrupa registros por curso, semestre, disciplina e período
    console.log('4. Agrupando registros...');
    const grouped = {};
    ...
    console.log(`   ✓ ${Object.keys(grouped).length} grupos (curso + semestre + disciplina + período)\n`);

    // 5. Processa cada grupo
    console.log('5. Importando notas...\n');
    ...
    // Cache de disciplinas por curso
    const courseDisciplinesCache = {};

    for (const [key, group] of Object.entries(grouped)) {
      const courseInfo = COURSES[group.course];
      if (!courseInfo) {
        console.log(`\n   ⚠ Curso não mapeado: ${group.course}`);
        continue;
      }

      console.log(`\n   Processando: ${courseInfo.name} - ${group.disciplineName} - ${group.semester}° sem - ${group.periodo}`);

      // Busca disciplinas do curso (com cache)
      if (!courseDisciplinesCache[courseInfo.id]) {
        const courseDisciplines = await db.sequelize.query(
          `SELECT d.id, d.name, d.code
           ...
           ORDER BY d.name`,
          {
            replacements: { courseId: courseInfo.id },
            type: db.sequelize.QueryTypes.SELECT
          }
        );
        courseDisciplinesCache[courseInfo.id] = courseDisciplines;
        console.log(`   ℹ Carregadas ${courseDisciplines.length} disciplinas do curso ${courseInfo.name}`);
      }

      const courseDisciplines = courseDisciplinesCache[courseInfo.id];
      ...
      if (!discipline) {
        ...
        try {
          discipline = await createDiscipline(group.disciplineName, courseInfo.id);

          // Adiciona a nova disciplina ao cache
      ...
      // Cria ou busca turma
      const classRecord = await getOrCreateDefaultClass(courseInfo.id, group.semester);
      ...
      for (const studentRecord of group.students) {
        try {
          ...
          if (studentRecord.teste !== null && studentRecord.teste !== undefined && studentRecord.teste !== '') {
            let testeValue = parseFloat(studentRecord.teste);
            if (!isNaN(testeValue)) {
              // Validação: nota deve estar entre 0 e 10
              if (testeValue < 0 || testeValue > 10) {
                console.log(`   ⚠ Nota inválida (teste=${testeValue}) para aluno ${studentRecord.matricula}, considerando 0`);
                testeValue = 0;
              }
              gradesToCreate.push({
          ...
          if (studentRecord.prova !== null && studentRecord.prova !== undefined && studentRecord.prova !== '') {
            let provaValue = parseFloat(studentRecord.prova);
            if (!isNaN(provaValue)) {
              // Validação: nota deve estar entre 0 e 10
              if (provaValue < 0 || provaValue > 10) {
                console.log(`   ⚠ Nota inválida (prova=${provaValue}) para aluno ${studentRecord.matricula}, considerando 0`);
                provaValue = 0;
              }
              gradesToCreate.push({
          ...
          if (studentRecord.resultado !== null && studentRecord.resultado !== undefined && studentRecord.resultado !== '') {
            let resultadoValue = parseFloat(studentRecord.resultado);
            if (!isNaN(resultadoValue)) {
              // Validação: nota deve estar entre 0 e 10
              if (resultadoValue < 0 || resultadoValue > 10) {
                console.log(`   ⚠ Nota inválida (resultado=${resultadoValue}) para aluno ${studentRecord.matricula}, considerando 0`);
                resultadoValue = 0;
              }
              gradesToCreate.push({
