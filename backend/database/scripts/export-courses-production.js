/**
 * Script para exportar dados da tabela courses de produção
 ...
 * 2. node backend/database/scripts/export-courses-production.js
 * 3. Arquivo courses_export.sql será gerado
 */
...
const OUTPUT_FILE = path.join(__dirname, '..', '..', 'courses_export.sql');
...
function generateInsertSQL(records) {
  ...
  return `INSERT INTO courses (${columns.join(', ')}) VALUES\n${values.join(',\n')};\n\n`;
}

async function exportcourses() {
  console.log('=== EXPORTAÇÃO DE courses DE PRODUÇÃO ===\n');
  ...
  try {
    ...
    if (totalRecords === 0) {
      console.log('⚠️  Tabela courses está vazia em produção!');
    ...
    // Cabeçalho do SQL
    stream.write('-- Exportação da tabela courses de produção\n');
    stream.write(`-- Data: ${new Date().toISOString()}\n`);
    stream.write(`-- Total de registros: ${totalRecords}\n\n`);
    stream.write('SET FOREIGN_KEY_CHECKS=0;\n\n');
    stream.write('-- Limpar tabela courses antes de importar\n');
    stream.write('TRUNCATE TABLE courses;\n\n');
    ...
    while (offset < totalRecords) {
      ...
      const [records] = await sequelize.query(
        `SELECT * FROM courses LIMIT ${BATCH_SIZE} OFFSET ${offset}`
      );
