/**
 * Script: import-grades-enfermagem.js
 * Descrição: Importa notas de avaliações do curso Técnico em Enfermagem
 *            da tabela boletim_novo do banco legado
 *
 * Origem: database/academico_bo.sql
 * Tabela: boletim_novo
 * Filtro: semestre IN ('Técnico em Enfermagem I', 'Técnico em Enfermagem II')
 *
 * Mapeamento:
 ...
 * - boletim_novo.semestre -> extraction semester (I, II)
 *
 * Como executar:
 * cd backend
 * node database/scripts/import-grades-enfermagem.js
 */
...
// ID do curso Técnico em Enfermagem
const COURSE_ID = 46;
const COURSE_NAME = 'Técnico em Enfermagem';

/**
 * Extrai o número do semestre do campo "semestre" do banco legado
 * Exemplo: "Técnico em Enfermagem I" -> 1
 *          "Técnico em Enfermagem II" -> 2
 *
 * @param {string} semestre - Campo semestre do banco legado
 * @returns {number|null} Número do semestre (1-2) ou null
 */
function extractSemester(semestre) {
  if (!semestre) return null;

  const match = semestre.match(/Técnico em Enfermagem\s+(I{1,2})/i);
  if (match) {
...
function parseBoletimNovoInserts(sqlContent) {
  ...
  while ((match = insertPattern.exec(sqlContent)) !== null) {
    ...
    while ((valueMatch = valuePattern.exec(valuesBlock)) !== null) {
      ...
      // Filtra apenas registros do curso Técnico em Enfermagem
      if (record.semestre && /Técnico em Enfermagem\s+I{1,2}/i.test(record.semestre)) {
...
async function importGrades() {
  console.log('\n========================================');
  console.log('IMPORTAÇÃO DE NOTAS - TÉCNICO EM ENFERMAGEM');
  console.log('========================================\n');
